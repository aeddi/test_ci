...
// Listener is a Nearby Tpt.Listener.
...
func newListener(localMa ma.Multiaddr, t *Transport) (*Listener, error) {
	...
	// 	return nil, errors.New("listener creation failed: can't start Nearby native driver")
	// }
...
func (l *Listener) Accept() (tpt.CapableConn, error) {
	for {
		...
		case req := <-l.inboundConnReq:
			...
			// If the Nearby handshake failed for some reason, Accept won't return an error
...
func (l *Listener) Addr() net.Addr {
	lAddr, _ := l.localMa.ValueForProtocol(nearby_ma.P_NEARBY)
	return &Addr{
		Address: lAddr,
	}
}
