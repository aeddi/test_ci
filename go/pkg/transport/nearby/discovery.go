...
func HandleFoundPeer(sRemotePID string) bool {
	...
	if err != nil {
		log.Error("discovery handle peer failed: wrong remote peerID")
		return false
	}

	remoteMa, err := ma.NewMultiaddr(fmt.Sprintf("/nearby/%s", sRemotePID))
	...
	if gListener == nil || gListener.ctx.Err() != nil {
		log.Error("discovery handle peer failed: listener not running")
		return false
	}
